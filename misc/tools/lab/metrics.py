...
for base_dir in os.listdir(raw_results_path):
    ...
    for param_dir in param_dirs:
        ...
        if policy_heuristic not in policy_heuristics: 
            continue
        elif policy_heuristic == 'max-lookup-delta-nearest':
            for domain_dir in os.listdir(raw_results_path + base_dir + '/' + param_dir):
                ...
                for problem_dir in sorted(os.listdir(raw_results_path + base_dir + '/' + param_dir + '/' + domain_dir), key = lambda problem_name: problem_name_to_float(problem_name)):
                    ...
                    if not (os.path.exists(results_csv_path) and os.path.exists(samples_csv_path)): 
                        continue
                    samples_file = open(samples_csv_path, 'r')
                    results_file = open(results_csv_path, 'r')
                    if samples_header in samples_file.readline() and results_header in results_file.readline():
                ...
                print(df)
                plot_data(df, domain_dir, min_h, max_h, x='n', y='k', params=param_dir)
        elif policy_heuristic == 'lookup-on-delta-nearest':
            ...
            for domain_dir in os.listdir(raw_results_path + base_dir + '/' + param_dir):
                ...
                for problem_dir in sorted(os.listdir(raw_results_path + base_dir + '/' + param_dir + '/' + domain_dir), key = lambda problem_name: problem_name_to_float(problem_name)):
                    ...
                    if not (os.path.exists(results_csv_path) and os.path.exists(delta_nearest_csv_path)): 
                        continue
                    results_file = open(results_csv_path, 'r')
                    delta_nearest_file = open(delta_nearest_csv_path, 'r')
                    if results_header in results_file.readline() and results_header in delta_nearest_file.readline():
                        results_df = pd.read_csv(results_csv_path)
