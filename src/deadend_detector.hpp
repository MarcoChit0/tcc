...
class DeadEndDetector
{
    private:
        const Task &task;
    public:
        void find_weak_alive_states(const set<State> &goal_states, map<State, vec<std::pair<State, Action>>> &reversed_edges, set<State> &weak_alive_states, vec<std::pair<State, Action>> &bad_state_action_pairs);
        ...
        void mark_goal_states_as_alive(const Task &task, const set<State> &states, map<State, vec<std::pair<State, Action>>> &reversed_edges, set<State> &goal_states, set<State> &non_goal_states);
